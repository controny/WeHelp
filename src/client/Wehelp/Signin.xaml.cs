...
/*
 * 登录界面，向服务器发送请求，检查用户密码是否匹配
 * 若匹配，获得该用户相应信息，跳转到主页面
 * 若不匹配则返还错误信息
 * 
 * By 许琦
 */
            ...
            using (HttpClient client = new HttpClient())
            {
                    ...
                    var kvp = new List<KeyValuePair<string, string>>
                {
                    ...
                    if (responceJson1["statusCode"].ToString() == "ok")
                    {
                        ...
                        var bg = responceJson["background"];
                        ...
                        if (bg != null)
                        {
                            regster.background = new BitmapImage(new Uri(bg.ToString()));
                        } else
                        {
                            regster.background = null;
                        }
                        if (headString !="")
