...
/*
 * 该界面实现了对于任务的分享，当跳转到该页面后实现对磁铁的更新，接受和删除功能
 * 若该用户为任务的发布者才可以删除任务，该用户不为任务的发布者才可以接受任务
 * 任务的分享可以通过邮件，Onedrive等应用
 * 所有的任务信息都是只读的，不能更改
 * 
    ...
    public sealed partial class MainPage : Page
    {
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (e.Parameter.GetType() == typeof(Models.Users))
            {
                ...
                im.ImageSource = currentUers.background;
                bg.Background = im;
        ...
        private void share_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (click != null)
            {
                ...
                {
                    Shared_content = "";
                    Shared_content += now.publisherId + " has a " + now.type + " task" + " whose details are " + now.details + ". And it is worth " + now.commission + " scores.";
        ...
        private void helpListViewItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (InlineHelpItemViewGrid.Visibility == Visibility.Visible)
            {
                ...
                inline_task_image.ImageSource = new BitmapImage(new Uri("ms-appx:///Assets/ItemType/" + helpItemViewModel.SelectedItem.type + ".png"));
        ...
        private async void changeBackground()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                HttpResponseMessage response = await client.PostAsync("http://paradox5.cn/userInfo" + "?userId=" + currentUers.userId, new FormUrlEncodedContent(kvp));
            }
        }
